...
export const handlerAppsUsers = [
  ...
  rest.get(('/api/apps/users'), (req, res, ctx) => {
    ...
    // filter users
    let filteredUsers = db.users.filter(user => ((user.fullName.toLowerCase().includes(queryLower) || user.email.toLowerCase().includes(queryLower)) && user.role === (role || user.role) && user.address === (address || user.address) && user.status === (status || user.status))).reverse()
  ...
  // Get Single User Detail by sdt
  rest.get(('/api/apps/users/:phoneNumber'), (req, res, ctx) => {
    const phoneNumber = req.params.id

    const user = db.users.find(e => e.phoneNumber === phoneNumber)
