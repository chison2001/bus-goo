...
// 👉 headers
const headers = [
  { title: 'Code', key: 'cusCode' },
  { title: 'Tên khách hàng', key: 'cusName' },
  { title: 'Doanh thu trước chiết khấu', key: 'ticketPrice' },
  { title: 'Chiết khấu', key: 'discount' },
  { title: 'Doanh thu sau chiết khấu', key: 'revenue' },
]

// 👉 Fetch Invoices

async function getRevenue() {
  const res = await $api('/api/report/sale-of-customer/page', {
    method: 'GET',
    params: {
...
async function download() {
  try {
    const response = await $api('/api/report/sale-of-customer', {
      method: 'GET',
      params: {
        fromDate: selectedFromDate.value,
        toDate: selectedToDate.value,
